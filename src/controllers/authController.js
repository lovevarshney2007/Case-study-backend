...
const sendOtpController = asyncHandler(async (req, res) => {
    ...
    await transporter.sendMail({
        from: `"Miles Matrix Security" <${process.env.EMAIL_USER}>`,
        to: email,
        subject: "Your Registration OTP",
        html: `<h3>Welcome to Miles Matrix!</h3>
               <p>Your OTP for registration is: <b style="font-size: 20px;">${otp}</b></p>
               <p>This OTP is valid for 5 minutes only.</p>`,
    });

    return res.status(200).json(new ApiResponse(200, {}, "OTP sent successfully to email"));
});

// Register User
const registerController = asyncHandler(async (req, res) => {
    const { userName, email, password, confirmPassword, otp } = req.body; // 'otp' add kiya

    // Validate fields
    if ([userName, email, password, confirmPassword, otp].some((field) => !field || field.trim() === "")) {
    ...
    // Sabse recent OTP nikalne ke liye sort({ createdAt: -1 }) use kiya
    ...
    // User banne ke baad DB se OTP delete kar do (Security best practice)
    ...
    return res.status(201).json(new ApiResponse(201, createdUser, "User verified and registered successfully"));
});
